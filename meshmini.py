...
class MiniBBS:
    ...
    def _send_paged(self, dest: Optional[str], lines: list, title=None):
        head = f"{title}\\n" if title else ""
        pages, cur = [], (head)
        for ln in lines:
    ...
    def _cmd_menu(self, frm):
        ...
        if row and row["v"]:
            self._send_paged(frm, [row["v"]], title=None)
    ...
    def _post_new(self, author, text, reply_to=None, do_sync=True):
        cur = self.db.cursor()
        cur.execute("INSERT INTO posts(ts,author,body,reply_to) VALUES(?,?,?,?)", (int(time.time()), author, text, reply_to))
        self.db.commit()
        pid = cur.lastrowid
    ...
    def _cmd_dm(self, frm, short, *text):
        ...
        if not target: self._send_text(frm, f"no node with short '{short}'"); return
        cur = self.db.cursor()
        ...
        cur.execute("INSERT INTO dm_out(to_id,body,created_ts,delivered_ts) VALUES(?,?,?,NULL)", (target, msg, int(time.time())))
        self.db.commit()
    ...
    def _deliver_queued(self, toId):
        ...
        for row in cur.execute("SELECT id,body FROM dm_out WHERE to_id=? AND delivered_ts IS NULL LIMIT 3", (toId,)):
            ...
            cur.execute("UPDATE dm_out SET delivered_ts=? WHERE id=?", (int(time.time()), row["id"]))
        self.db.commit()
    ...
    def _replicate_new_post(self, pid, body, author, reply_to):
        uid = gen_uid()
        header = f\"{SYNC_TAG} POST uid={uid} id={pid} ts={int(time.time())} by={author} r={(reply_to or '-')}\"
    ...
    def _handle_sync(self, fromId, text):
        ...
        if cmd == "POST":
            ...
            self.db.execute("INSERT OR IGNORE INTO seen_uids(uid,ts) VALUES(?,?)", (uid, int(time.time())))
            self.db.execute("INSERT OR IGNORE INTO rxparts(uid,total,got,data,from_id,created_ts) VALUES(?,?,?,?,?,?)",
                            (uid, total, 0, "", fromId, int(time.time())))
        ...
        if cmd == "END":
            ...
            if self.db.execute("SELECT 1 FROM applied_uids WHERE uid=?", (uid,)).fetchone():
                # already applied
                self.db.execute("DELETE FROM rxparts WHERE uid=?", (uid,)); self.db.commit(); return
            ...
            self.db.execute("INSERT OR IGNORE INTO applied_uids(uid,ts) VALUES(?,?)", (uid, int(time.time())))
